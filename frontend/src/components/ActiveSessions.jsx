...
function ActiveSessions({ sessions, isLoading, isUserInSession }) {
  return (
    <div className="lg:col-span-2 card h-full" data-reveal>
        ...
        <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
          {isLoading ? (
            <div className="flex items-center justify-center py-20">
              <LoaderIcon className="size-10 animate-spin text-white/70" />
            </div>
          ) : sessions.length > 0 ? (
            sessions.map((session, index) => {
              const participantCount = (session.participants?.length || 0) + 1;
              const maxParticipants = session.maxParticipants || 2;
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-4 text-sm opacity-80">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-xs">{participantCount}/{maxParticipants}</span>
                        </div>
                        {isFull && !isUserInSession(session) ? (
                          <span className="ca-badge">FULL</span>
                        ) : (
                          <span className="ca-badge">OPEN</span>
                        )}
                      </div>
                    </div>
                  </div>
            ...
            <div className="text-center py-16">
              <div className="w-20 h-20 mx-auto mb-4 border border-[var(--border-subtle)] rounded-3xl flex items-center justify-center">
                <SparklesIcon className="w-10 h-10 text-white/50" />
              </div>
